...
function EditSecondInvoice() {
    ...
    setFormUpdateData((prevData) => {
      if (index !== undefined) {
        ...
        const totalAmount = updatedItems.reduce((total, item) => {
          ...
          return (
            total + parseFloat(item.quantity || 0) * parseFloat(priceEach || 0)
          );
        }, 0);
  ...
  const handleBillToEnterKey = (e, index) => {
    if (e.key === 'Enter') {
      ...
      setTimeout(() => {
        fieldRefs.current[newFieldIndex] &&
          fieldRefs.current[newFieldIndex].focus();
      }, 0);
    }
  };

  useEffect(() => {
    fieldRefs.current = fieldRefs.current.slice(
      0,
      formUpdateData.bill_to.length
    );
      ...
      switch (currentField) {
        ...
        case 'price_each':
          if (currentIndex === formUpdateData.items.length - 1) {
            // Check if it's the last 'price_each' in the last item
  ...
  return (
    <div id='invoice-generated'>
      <div className='row'>
        ...
        <div
          className='col-3'
          style={{ marginTop: '55px', marginLeft: '60px' }}
        >
      ...
      >
        <div id='pdf'>
          ...
          <form>
            <div
              className='row bill_to_div px-3'
              style={{ border: '2px solid white', marginTop: '15px' }}
            >
            ...
            <div className='last-row' style={{ marginTop: '-20px' }}>
              <div className='row po_details_div px-3'>
                ...
                <div className='col-md-2' style={{ textAlign: 'center' }}>
                  <span style={{ fontWeight: '800', marginLeft: '15px' }}>
                    Type of Work
                  </span>
                  <input
                    id='type_of_work'
              ...
              <div
                className='row item_details_div px-3'
                                        ...
                                        <Autocomplete
                                          ...
                                          renderInput={(params) => (
                                            <TextField
                                              ...
                                              variant='standard'
                                              inputRef={(el) =>
                                                (fieldRefs.current[fieldIndex] =
                                                  el)
                                              }
                                              onKeyDown={(e) =>
                                                handleBillToEnterKey(
                                                  e,
                                                  fieldIndex
                                                )
                                              }
                    ...
                    >
                      ...
                      <div className='col-md-2'>
                        <TextField
                          ...
                          onKeyPress={(e) => handleLotNoKeyPress(e, index)}
                          onKeyDown={(event) =>
                            handleEnterKeyPress(event, 'price_each', index)
                          }
                        />
                      </div>
                ...
                style={{
                  marginTop:
                    formUpdateData.items.length === 2
                      ? '1000px'
                      : formUpdateData.items.length >= 3 &&
                        formUpdateData.items.length <= 5
                      ? '600px'
                      : formUpdateData.items.length >= 6 &&
                        formUpdateData.items.length <= 8
                      ? '500px'
